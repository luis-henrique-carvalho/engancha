...
export function WorkspaceShell({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(undefined)
    void apiFetch<ActiveWorkspaceResponse>('/workspaces/bootstrap', { method: 'POST' })
      .then((value) => {
  ...
  if (session.isPending || loading) return <WorkspaceState title="Preparando seu workspace…" />
  ...
  return (
    <AuthenticatedLayout
      user={{ name: user?.name || 'Minha conta', email: user?.email || '', image: user?.image }}
      workspace={workspace}
