...
status: "in-progress"
type: "HITL"
parent: "docs/phases/phase-02-authentication-workspaces/002-authentication-workspaces-organization-isolation/prd.md"
blocked_by:
  - "docs/phases/phase-02-authentication-workspaces/002-authentication-workspaces-organization-isolation/tickets/001-sign-up-and-email-password-session-with-prisma-and-better-auth.md"
user_stories: [1, 2, 3]
---
...
Implementado em `packages/contracts/src/index.ts`, `apps/api/src/auth/auth.ts`, `apps/api/src/email` e `apps/worker/src/email`. O job `EmailDeliveryJob v1` suporta verificação e reset, usa retry BullMQ e transport mockado quando `RESEND_API_KEY` não existe; logs omitem destinatário, URL e token. Em desenvolvimento/teste, o worker mantém somente `{ type, actionUrl }` no Redis por uma hora e `GET /api/v1/dev/email-outbox/:correlationId` permite recuperar o link sem expor o destinatário; em produção a rota retorna `404` e o conteúdo não é armazenado. `tests/phase2-e2e.test.mjs` confirma o cadastro, o link enfileirado e a confirmação de e-mail contra PostgreSQL e Redis locais. Smoke Resend real permanece pendente de credenciais autorizadas.
