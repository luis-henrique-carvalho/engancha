...
- A associação a `ChannelConnection` foi resolvida posteriormente pela PRD [Conexões de canais e direcionamento de automações](../../phase-future-instagram-meta/007-channel-connections-and-automation-targeting/prd.md): cada revisão real terá uma conexão em seu alvo, enquanto o fluxo simulado atual permanece com conexão nula. OAuth, posts reais e lifecycle da conexão continuam na integração real.
